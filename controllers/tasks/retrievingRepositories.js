...
const retrievingRepositories = () => {
    ...
    const func = async () => {
        try {
            const dateRange = new Date(new Date() - MINUTES_AGO * 60 * 1000).toISOString();
            const reps = await githubApi.get(`/search/repositories?q=created:>${dateRange}&sort=stars&order=desc&per_page=5`)
